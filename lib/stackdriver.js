...
function StackdriverBackend(startupTime, config, emitter) {
	...
	if (this.sourceFromPrefix) {
		...
		var sys = require('sys');
	 	...
	 	util.log('Source param set to ' + this.source + ' prior to detection');
	 	
	 	var instanceRequest = '';
	 	if (this.source == 'detect-aws') {
	 		instanceRequest = 'wget -q -O - http://169.254.169.254/latest/meta-data/instance-id';
	 	} else if (this.source == 'detect-gce') {
	 		instanceRequest = 'wget -q -O - http://metadata/computeMetadata/v1/instance/id --header "X-Google-Metadata-Request: true"';
	 	} else {
	 		// If we're not detecting a source, run something that returns no output
	 		instanceRequest = '/bin/true'
	 	}

	 	var child = exec(instanceRequest, function(error, stdout, stdin) {
	 		if (stdout) {
	 			this.detectId = stdout;
	 		}
	 		util.log('Source param set to ' + this.source + ', all points sent to Stackdriver will be associated with that instance');

	 		// attach once the source is reset
